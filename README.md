...
![architecture](open-on-demand-on-aws/images/architecture.png)
...
2. Deploy the all-in-one stack [ood_full.yml](open-on-demand-on-aws/assets/cloudformation/ood_full.yml) via CloudFormation.  
...
1. Deploy Infrastructure (*Networking, and Managed Active Directory*): [infra.yml](open-on-demand-on-aws/assets/cloudformation/infra.yml)
2. Deploy Slurm Accounting Database (_only if integrating with ParallelCluster_): [slurm_accounting_db.yml](open-on-demand-on-aws/assets/cloudformation/slurm_accounting_db.yml)
3. Deploy Open OnDemand: [ood.yml](open-on-demand-on-aws/assets/cloudformation/ood.yml)
...
3. Add [configure_pam_slurm_adopt.sh](open-on-demand-on-aws/scripts/configure_pam_slurm_adopt.sh) to **OnNodeConfigured** in the [CustomActions](https://docs.aws.amazon.com/parallelcluster/latest/ug/Scheduling-v3.html#Scheduling-v3-SlurmQueues-CustomActions) section.  
...
**Option 1**: Use [scripts/deploy_pcs.sh](open-on-demand-on-aws/scripts/deploy_pcs.sh) script to deploy [assets/cloudformation/pcs-starter.yml](open-on-demand-on-aws/assets/cloudformation/pcs-starter.yml).  This script is a helper utility to help deploy the `pcs-starter` template by pulling the parameters from CloudFormation outputs.
...
**Option 2**: Manually deploy [assets/cloudformation/pcs-starter.yml](open-on-demand-on-aws/assets/cloudformation/pcs-starter.yml) via **CloudFormation**
...
The [scripts/s3_script_runner.sh](open-on-demand-on-aws/scripts/s3_script_runner.sh) script uses AWS Systems Manager (SSM) to send a command to the Open OnDemand EC2 instance. This command executes a configuration script that sets up both `sackd` and `slurm` services to work with your PCS cluster.
...
See [CONTRIBUTING](open-on-demand-on-aws/CONTRIBUTING.md) for more information.

## 📄 License

This library is licensed under the MIT-0 License. See the [LICENSE](open-on-demand-on-aws/LICENSE) file for details.
